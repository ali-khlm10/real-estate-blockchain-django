...
@csrf_exempt
def create_signature_to_accept_buy_request(request: HttpRequest):
    if request.method == "POST":
        try:
            data: dict = json.loads(request.body)
            token_id: str = data.get("token_id")
            ...
            if current_token:
                ...
                message_str: str = json.dumps(accept_buy_request_information)
                sender_private_key_str: str = request.user.wallet.private_key[2:-1]
                ...
                return JsonResponse({
                    "status": True,
                    "accept_buy_request_information": accept_buy_request_information,
                    "signature": signature,
                    "message": "signature successful."
                })
...
def accepting_buy_request(request: HttpRequest):
    if request.method == "POST":
        data: dict = json.loads(request.body)
        # print(data)
        if verify_accepting_buy_request_transaction(data=data):
...
def verify_accepting_buy_request_transaction(data: dict):

    result: bool = accept_buy_request_signature_verification(data=data)
    # print(result)
    if result:
        verification_counter = 0
        nodes: nodeModel = nodeModel.objects.filter(is_disable=False).all()
        for node in nodes:
            ...
            csrf_token = csrf.get_token(request=HttpRequest())
            response = requests.post(
                f"{node.node_url}/verification_accept_buy_request_transaction_by_nodes/",
                data=json.dumps(data),
                headers={"Content-Type": "application/json",
                         "X-CSRFToken": csrf_token})
            if response.json()["status"]:
...
def accept_buy_request_signature_verification(data: dict):
    accept_buy_request_signature = data.get("signature")
    accept_buy_request_information: dict = data.get(
        "accept_buy_request_information")
    message_str: str = json.dumps(accept_buy_request_information)

    current_sender: userModel = userModel.objects.filter(
        wallet__wallet_address__iexact=accept_buy_request_information.get("sender")).first()
    ...
    verify_result: bool = verify_signature(
        public_key=sender_public_key, signature=bytes.fromhex(accept_buy_request_signature), message=message_str)
...
@csrf_exempt
def verification_accept_buy_request_transaction_by_nodes(request: HttpRequest):
    if request.method == "POST":
        data: dict = json.loads(request.body)
        # print(data)
        result: bool = accept_buy_request_signature_verification(data=data)
...
def add_accept_buy_request_trx_and_mine_block(data: dict):
    ...
    buy_request: buyRequestModel = buyRequestModel.objects.filter(
        token=token, buy_request_from=buyer).first()
